...
[![](https://i.imgur.com/6bXMuEI.png)](https://i.imgur.com/6bXMuEI.png)

給一個一維整數陣列 `heights`，代表每棟建築物的高度，以及二維整數陣列 `queries`，代表 Alice 和 Bob（以下簡稱 `a` 和 `b`）的所在建築物的 index。
...
前兩種情況可以簡單判斷來處理，第三種情況則需要使用 monotonic stack 來處理，用一個一維陣列 `highest` 來記錄每個建築物右邊第一個比它高的建築物 index，若沒有則設為 -1。令 $j = highest[b]$ ，從 `j` 開始往右搜尋，如果 $heights[j] > heights[a]$ ，則 $res[i] = j$ ，否則根據 monotonic stack 的特性，`j` 到 `highest[j]` 之間的建築物都不會比 `j` 高，因此可以將 $j = highest[j]$ 來跳躍右邊的建築物往右搜尋。如果都找不到則 $res[i] = -1$ 。
...
[![](https://i.imgur.com/pYSwPev.png)](https://i.imgur.com/pYSwPev.png)
...
[![](https://i.imgur.com/HGgAN4M.png)](https://i.imgur.com/HGgAN4M.png)
...
[![](https://i.imgur.com/ZczUWH5.png)](https://i.imgur.com/ZczUWH5.png)
