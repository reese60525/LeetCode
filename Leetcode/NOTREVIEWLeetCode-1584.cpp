 ...
 *
 * 題目解釋:
 * 給n組座標，請算出走訪所有座標所需要的最小路徑，並且不能重複走訪一個點，路徑長以manhattan distance來計算，
 * 假設有兩點(x1, y1)、(x2, y2)，manhattan distance為|x1 - x2| + |y1 - y2|。
 *
 * 思路:
 * 標準的Minimal Spanning  Tree(最小生成樹MST)問題，可以用Prime或是Kruskal演算法來處理，由於測資最多達n = 1000，推薦使用
 * Prime，因為Kruskal是以所有路徑的可能從最小開始走訪，有n的點就需要n * (n-1) / 2筆路徑資料，格式為{點1之座標, 點2之座標, weight}
 * ，約為n^2 / 2，再加上排序的話總體時間複雜度高達O(n^2 * long n)，而Prime由於是根據目前以走訪的點，從這些點的可走路徑來判斷要走哪，
 * 因此路徑數據量是慢慢遞增，資料式通常為(下個目的地點, weight)，最多有2 * n筆資料，而資料是每走過一個點就會新增，所以通常用priority
...
class Solution {
  public:
    ...
    int minCostConnectPoints(std::vector<std::vector<int>> &points) {
        int n = points.size(), res = 0, visited_count = 0; // visited_count用以計算目前走訪了幾個點
        std::vector<bool> visited(n, false);               // 紀錄每個點的走訪狀態，走訪過的為true
        std::priority_queue<std::pair<int, int>, std::vector<std::pair<int, int>>, cmp> pq; // distance最短的在pq中權重最高

        pq.push(std::pair(0, 0)); // 先放入一開始要走訪的點，並先把distance設為0，因為此時還沒和任何點相連
        ...
        while (!pq.empty() && visited_count < n) {
            ...
            // 把與目前該點所有連接的線路放入pq
            for (int j = 0; j < n; ++j) {
                if (!visited[j]) { // 若該線路的點走訪過則不再次走訪，避免graph形成環狀迴圈
                    int weight = std::abs(points[cur_node][0] - points[j][0]) + std::abs(points[cur_node][1] - points[j][1]);
...
//         std::vector<std::vector<int>> edges(n * (n - 1) / 2, std::vector<int>(3, 0));
