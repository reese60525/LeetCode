...
這時的 `index` 為 **-1**，因此可以將 `dp` 的大小設為 `dp[nums.size() + 1][sum + 1]`， $nums.size() + 1$ 多出來的 $1$ 是為了處理邊界條件中 $index = -1$ 的情況，防止 `index` out of range，而 $sum + 1$ 是因為 `sum` 的範圍為 $[0, sum]$ 共 $sum + 1$ 種情況。
...
`dp[i+1][2]` 由 `dp[i][2]` 和 `dp[i][0]` 相加得到， $dp[i][0] = 1$ ， $dp[i][2] = 3$ ，所以 $dp[i+1][2] = 4$ 。
...
`dp[i+1][3]` 由 `dp[i][3]` 和 `dp[i][1]` 相加得到， $dp[i][1] = 2$ ， $dp[i][3]= 4$ ，所以 $dp[i+1][3] 6$ 。
...
**接下來是重點**  
`dp[i+1][4]` 由 `dp[i][4]` 和 `dp[i][2]` 相加得到，但是 `dp[i][2]` 的值已經被 `dp[i+1][2]` 覆蓋，從原本的 $3$ 變成 $4$，所以此時 $dp[i+1][4] = 9$ 是**錯誤的**，正確的值應該是 $8$。
