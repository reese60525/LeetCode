  ...
  public:
    ...
    bool canPartition(std::vector<int> &nums) {
        ...
        if (target & 1)
            return false;
        target /= 2;
        dp1.resize(nums.size(), std::vector<int>(target + 1, -2));
...
int main() {
    Solution solution;
    std::vector<int> nums {10, 9, 9, 9, 9, 8, 7, 3, 1, 1}; // 1,1,3,9,9,10   7,8,9,9
